    ...
    ILogger<EventBusBatchHandler> logger) : IEventBatchHandler
{
    ...
    private async Task HandleEvent(IEventEnvelope eventEnvelope, CancellationToken token)
    {
        try
        {
            await activityScope.Run($"{nameof(EventBusBatchHandler)}/{nameof(HandleEvent)}",
                async (_, ct) => await eventBus.Publish(eventEnvelope, ct).ConfigureAwait(false),
